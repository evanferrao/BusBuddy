 ...
 */
export async function savePassengerData(data: {
  stopId?: string;
  stopName?: string;
}): Promise<void> {
  try {
    await AsyncStorage.setItem(STORAGE_KEYS.PASSENGER_DATA, JSON.stringify(data));
  } catch (error) {
 ...
 */
export async function getPassengerData(): Promise<{
  stopId?: string;
  stopName?: string;
} | null> {
  try {
    const data = await AsyncStorage.getItem(STORAGE_KEYS.PASSENGER_DATA);
    return data ? JSON.parse(data) : null;
  } catch (error) {
